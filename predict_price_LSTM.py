...
# Extracting Date and Close Price only
tesla = tesla[['Date', 'Close']]

# Selecting the stationary part of the dataset. 3 years
new_tesla = tesla.loc[884:1639]

# Feature preprocessing
new_tesla = new_tesla.drop('Date', axis = 1) # 1 is for representing column 0 for index drop from column or index
new_tesla = new_tesla.reset_index(drop = True) # drop == True : insert the index in the DF
T = new_tesla.values
T = T.astype('float32')
T = np.reshape(T, (-1, 1)) # numpy.reshape(array, shape, order = 'C'). -1 means number of rows will be adjusted by
                            # number of column. 1 in this case
...
best_model = load_model('saved_models/model_epoch_96.hdf5')
